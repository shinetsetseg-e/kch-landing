...
import { Activity, ArrowDown, BarChart3, ChevronRight, Cpu, Globe, Lock, Shield } from 'lucide-react';
...
const LogoIcon = ({ className }: { className?: string }) => (
  <svg width="175" height="40" viewBox="0 0 175 40" fill="none" xmlns="http://www.w3.org/2000/svg" className={className}>
    ...
    <path d="M115.796 39.782H112.659V22.7239H115.796V39.782Z" fill="currentColor"></path>
    <path d="M169.531 26.2706C171.208 26.2706 172.518 26.7698 173.47 27.7641C174.422 28.7585 174.886 30.1459 174.886 31.9298V39.782H171.729V32.2753C171.729 31.2595 171.477 30.4442 170.969 29.8509C170.461 29.2578 169.706 28.9635 168.686 28.9635C167.803 28.9635 167.043 29.2792 166.437 29.8851C165.831 30.4911 165.527 31.3874 165.527 32.5438V39.7692H162.416V26.292H165.498V28.131C165.942 27.5678 166.505 27.1155 167.196 26.7784C167.887 26.4456 168.656 26.2706 169.531 26.2706Z" fill="currentColor"></path>
    <path d="M143.37 21.256C143.989 21.256 144.462 21.4395 144.812 21.7937C145.158 22.1522 145.328 22.6257 145.328 23.2103C145.328 23.795 145.158 24.2605 144.812 24.619C144.462 24.9731 143.988 25.1482 143.37 25.1482C142.743 25.1482 142.256 24.9731 141.923 24.619C141.586 24.2605 141.415 23.795 141.415 23.2103C141.415 22.6257 141.586 22.1522 141.945 21.7937C142.303 21.4395 142.777 21.256 143.37 21.256Z" fill="currentColor"></path>
  </svg>
);
...
const Hero: React.FC<Props> = ({ t }) => {
  ...
  return (
    <section 
      ref={containerRef}
      id="home" 
      className="relative min-h-[100dvh] lg:h-screen w-full flex items-center justify-center overflow-hidden bg-white dark:bg-[#030712] transition-colors duration-1000 py-12 lg:py-0"
    >
      ...
      <div className="max-w-[1600px] mx-auto px-6 md:px-12 lg:px-16 w-full grid grid-cols-1 lg:grid-cols-12 items-center gap-6 md:gap-10 lg:gap-16 relative z-20">
        
        {/* Modern Typography Column */}
        <div className="lg:col-span-6 space-y-4 md:space-y-6 lg:space-y-8 order-2 lg:order-1">
          <AnimatePresence mode="wait">
            <motion.div
              key={activeIndex}
            ...
            >
              <div className="flex items-center space-x-3">
                <span className="px-2 py-0.5 md:px-3 md:py-1 bg-blue-500/10 text-blue-600 dark:text-blue-400 text-[9px] md:text-[10px] font-black uppercase tracking-[0.2em] rounded-md border border-blue-500/10">
                  {currentSlide.tag}
                </span>
                <div className="h-px w-6 md:w-8 bg-slate-200 dark:bg-slate-800" />
                <span className="text-[9px] md:text-[10px] font-bold text-slate-400 uppercase tracking-widest">{currentSlide.systemId}</span>
              </div>

              <h1 className="text-2xl sm:text-3xl md:text-5xl lg:text-6xl font-bold tracking-tight text-slate-900 dark:text-white leading-[1.1] whitespace-pre-line">
                {currentSlide.title}
              </h1>

              <p className="text-sm md:text-base lg:text-xl text-slate-500 dark:text-slate-400 font-light max-w-lg leading-relaxed">
                {currentSlide.desc}
              </p>

              <div className="flex flex-wrap gap-3 md:gap-4 pt-1 md:pt-2">
                {currentSlide.features.map((f, i) => (
                  <div key={i} className="flex items-center gap-2 text-[8px] sm:text-[10px] lg:text-[11px] font-bold text-slate-400 dark:text-slate-50 uppercase tracking-widest">
              ...
              <div className="pt-2 md:pt-4 lg:pt-6">
                <button 
                  ...
                  className="group relative px-6 py-3 md:px-10 md:py-5 bg-slate-950 dark:bg-white text-white dark:text-slate-950 rounded-xl md:rounded-2xl font-bold text-[10px] md:text-[12px] uppercase tracking-[0.2em] transition-all hover:scale-105 active:scale-95 flex items-center gap-3 md:gap-4 shadow-xl shadow-blue-500/5"
                >
        ...
        <div className="lg:col-span-6 flex justify-center items-center order-1 lg:order-2">
          <AnimatePresence mode="wait">
            <motion.div
              ...
              className="relative w-full aspect-[4/3] max-w-[320px] sm:max-w-[450px] lg:max-w-[550px] group scale-90 sm:scale-100"
            >
              {/* Premium Dashboard Shell */}
              <div className="absolute inset-0 bg-white dark:bg-slate-900/80 backdrop-blur-2xl border border-slate-200 dark:border-white/10 rounded-[1.5rem] md:rounded-[2.5rem] shadow-2xl overflow-hidden transition-all duration-700">
                
                {/* Visual Header */}
                <div className="flex justify-between items-center p-3 md:p-6 border-b border-slate-100 dark:border-white/5 bg-slate-50/50 dark:bg-slate-900/50">
                  <div className="flex gap-1.5 md:gap-2">
                    <div className="w-1.5 h-1.5 md:w-2.5 md:h-2.5 rounded-full bg-red-400/20" />
                    <div className="w-1.5 h-1.5 md:w-2.5 md:h-2.5 rounded-full bg-amber-400/20" />
                    <div className="w-1.5 h-1.5 md:w-2.5 md:h-2.5 rounded-full bg-emerald-400/20" />
                  </div>
                  <LogoIcon className="h-2.5 md:h-4 w-auto dark:text-white text-slate-900 opacity-60" />
                </div>

                <div className="p-4 md:p-10 space-y-4 md:space-y-10">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3 md:gap-6">
                      <div className="w-10 h-10 md:w-16 md:h-16 rounded-lg md:rounded-2xl bg-blue-600 flex items-center justify-center text-white shadow-lg transition-transform group-hover:scale-110 duration-500">
                        {React.cloneElement(currentSlide.icon as React.ReactElement<any>, { size: 18, className: "md:w-6 md:h-6" })}
                      </div>
                      <div className="space-y-0.5 md:space-y-1">
                        <h4 className="text-[10px] md:text-sm font-bold dark:text-white text-slate-900 uppercase tracking-tight">Active Instance</h4>
                        <p className="text-[7px] md:text-[10px] text-slate-400 uppercase tracking-widest font-black">Cluster: GLOBAL-01</p>
                      </div>
                    </div>
                  ...
                  <div className="grid grid-cols-2 gap-3 md:gap-6">
                    <div className="p-3 md:p-6 rounded-xl md:rounded-2xl bg-slate-50 dark:bg-white/5 border border-slate-100 dark:border-white/5 space-y-2 md:space-y-4">
                      <BarChart3 size={14} className="text-slate-400 md:w-[18px] md:h-[18px]" />
                      <div className="space-y-1.5 md:space-y-2">
                         <div className="h-0.5 md:h-1 w-full bg-slate-200 dark:bg-slate-800 rounded-full">
                           <motion.div initial={{ width: 0 }} animate={{ width: '85%' }} transition={{ duration: 2 }} className="h-full bg-blue-600" />
                         </div>
                         <p className="text-[6px] md:text-[8px] font-black uppercase text-slate-400 tracking-widest">Efficiency 85%</p>
                      </div>
                    </div>
                    <div className="p-3 md:p-6 rounded-xl md:rounded-2xl bg-slate-50 dark:bg-white/5 border border-slate-100 dark:border-white/5 space-y-2 md:space-y-4">
                      <Activity size={14} className="text-slate-400 md:w-[18px] md:h-[18px]" />
                      <div className="space-y-1.5 md:space-y-2">
                         <div className="h-0.5 md:h-1 w-full bg-slate-200 dark:bg-slate-800 rounded-full">
                           <motion.div initial={{ width: 0 }} animate={{ width: '92%' }} transition={{ duration: 2, delay: 0.2 }} className="h-full bg-blue-600" />
                         </div>
                         <p className="text-[6px] md:text-[8px] font-black uppercase text-slate-400 tracking-widest">Uptime 99.9%</p>
                      </div>
                    </div>
                  </div>

                  <div className="pt-3 md:pt-6 border-t border-slate-100 dark:border-white/5 flex items-center justify-between">
                    <div className="flex -space-x-1.5 md:-space-x-2">
                       {[1,2,3].map(i => (
                         <div key={i} className="w-5 h-5 md:w-8 md:h-8 rounded-full bg-slate-200 dark:bg-slate-800 border md:border-2 border-white dark:border-slate-900 flex items-center justify-center text-[6px] md:text-[8px] font-black uppercase text-slate-900 dark:text-white">U{i}</div>
                       ))}
                    </div>
                    <Lock size={10} className="text-slate-300 dark:text-slate-700 md:w-[14px] md:h-[14px]" />
                  </div>
                </div>

                {/* Ambient Soft Glow */}
                <div 
                  className="absolute -bottom-20 -right-20 w-40 h-40 md:w-80 md:h-80 rounded-full blur-[60px] md:blur-[120px] opacity-10 transition-colors duration-1000"
              ...
              <motion.div 
                ...
                className="absolute -z-10 -top-6 -left-6 md:-top-12 md:-left-12 opacity-5 dark:opacity-10"
              >
                <Cpu size={40} className="md:w-[120px] md:h-[120px]" />
              </motion.div>
            </motion.div>
          </AnimatePresence>
        </div>
      ...
      {/* Modern Horizontal Navigation - Adjusted position for small screens */}
      <div className="hidden sm:flex absolute bottom-8 lg:bottom-16 right-6 lg:right-16 items-center gap-6 md:gap-12 z-40">
        <div className="flex gap-3 md:gap-4">
          {slides.map((_, i) => (
            ...
            >
              <div className={`h-1 md:h-1.5 transition-all duration-500 rounded-full ${i === activeIndex ? 'w-8 md:w-12 bg-blue-600' : 'w-2 md:w-4 bg-slate-200 dark:bg-slate-800 group-hover:bg-slate-400'}`} />
            </button>
          ))}
        </div>
        <div className="h-4 md:h-6 w-px bg-slate-200 dark:bg-slate-800" />
        <span className="text-[8px] md:text-[10px] font-black text-slate-400 uppercase tracking-[0.4em]">0{activeIndex + 1} / 0{slides.length}</span>
      </div>

      {/* Mobile Slide Dots */}
      <div className="flex sm:hidden absolute bottom-16 left-1/2 -translate-x-1/2 gap-2.5 z-40">
        {slides.map((_, i) => (
          <button 
            key={i} 
            onClick={() => !isLocked && setActiveIndex(i)}
            className={`w-1.5 h-1.5 rounded-full transition-all ${i === activeIndex ? 'w-6 bg-blue-600' : 'bg-slate-300 dark:bg-slate-700'}`}
          />
        ))}
      </div>

      {/* Subtle Scroll Indicator - Centered vertically and horizontally across full width */}
      <motion.div 
        animate={{ y: [0, 6, 0] }}
        transition={{ duration: 2, repeat: Infinity }}
        onClick={() => document.getElementById('about')?.scrollIntoView({ behavior: 'smooth' })}
        className="absolute bottom-4 md:bottom-10 left-0 right-0 flex flex-col items-center justify-center cursor-pointer text-slate-300 hover:text-blue-600 transition-colors z-40"
      >
        <span className="text-[6px] md:text-[8px] font-black uppercase tracking-[0.6em] pl-[0.6em] text-center w-full">Initialize Scroll</span>
        <ArrowDown size={16} className="md:w-[24px] md:h-[24px] mt-2" strokeWidth={1} />
      </motion.div>
    </section>
  );
};
